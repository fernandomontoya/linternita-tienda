...
export default function ProductCardDb({ product }: { product: DbProduct }) {
  const price = new Intl.NumberFormat("es-MX", { style: "currency", currency: "MXN", minimumFractionDigits: 0 }).format(product.price);
      ...
      <div className="card-hover bg-white rounded-2xl overflow-hidden shadow-sm border border-[#E8C97A]/20">
        <div className="relative aspect-square bg-[#F9F0E6] overflow-hidden">
          ...
          {product.stock <= 5 && product.stock > 0 && (
            <span className="absolute top-3 left-3 bg-[#D4889A] text-white text-[10px] font-semibold px-2 py-0.5 rounded-full">Pocas piezas</span>
          )}
          {product.stock === 0 && (
            <span className="absolute top-3 left-3 bg-gray-400 text-white text-[10px] font-semibold px-2 py-0.5 rounded-full">Agotado</span>
          )}
        </div>
        <div className="p-4">
          <p className="text-[10px] uppercase tracking-widest text-[#C9A84C] font-semibold mb-1">{product.category}</p>
          ...
          <div className="flex items-center justify-between">
            <span className="text-lg font-bold text-[#C9A84C]">{price}</span>
