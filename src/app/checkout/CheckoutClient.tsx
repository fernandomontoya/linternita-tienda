...
const inputCls = "w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#C9A84C] focus:ring-2 focus:ring-[#C9A84C]/10 bg-white";
...
export default function CheckoutClient({ userEmail, defaultValues }: { userEmail: string; defaultValues: DefaultValues }) {
  ...
  const saveOrder = async (paymentMethod: string) => {
    ...
    // Guardar/actualizar perfil con los datos más recientes
    if (user) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12">
      ...
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-bold text-[#2C1810]">Finalizar Pedido</h1>
        <span className="text-xs text-[#2C1810]/40 hidden sm:block">{userEmail}</span>
      </div>
      ...
      {/* Datos — pre-cargados si ya los tiene */}
      <div className="bg-white rounded-2xl p-6 border border-[#E8C97A]/20 mb-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-semibold text-[#2C1810]">Tus datos</h2>
          {(defaultValues.nombre || defaultValues.telefono) && (
      ...
      <div className="bg-[#F9F0E6] rounded-2xl p-6 border border-[#E8C97A]/30 space-y-3">
        <p className="text-sm font-semibold text-[#2C1810] mb-4">Confirma tu pedido</p>
        <p className="text-xs text-[#2C1810]/60 mb-2">
          Te enviaremos los datos para transferencia bancaria por WhatsApp.
        </p>
